# 17-State Education Data — Coverage Matrix & Collection Plan

Reconnaissance for expanding the pipeline from AZ + FL to 17 states:
...
plus **Kansas, Mississippi, New Mexico, Idaho**.
...
| **Arizona** | `azreportcards.azed.gov` API; `azed.gov` accountability / hns-frp / sppapr; finance portal; ADE Workforce (Power BI) | **API** + STATIC + Dashboard | state→district→school | Re-audited Aug 2026 after review flagged gaps. `azed.gov/data/public-data-sets` is a **hub**, not a file page — it links to the pages that publish files, and we had crawled only one of them. Now seeded from all three (`azed_data_pages.py`): accountability-research (132 files — assessment, graduation cohorts, dropout, Oct-1 enrollment), `hns/frp` (17 — free & reduced-price percentages, SY2019-2026), `specialeducation/sppapr` (72 — IDEA Part B indicator profiles FFY2020-2024, filed by category). **Two bugs the audit surfaced.** The accountability page once served files via `GetDocumentFile?id=N`; naming by URL basename made **69 downloads overwrite one path**, and the skip-existing check then logged all 69 as `skipped_existing` — the manifest read 132/132 while nine graduation cohorts (2010-2018), five dropout years (2015-2019), six Oct-1 enrollment years (2010-2016) and three AZELLA years were absent. Separately, Cloudflare 536-byte error pages were being saved as `.pdf` and hashed as successes (12 removed). Both are fixed in `common/static_site.py` (`choose_dest`, `looks_like_html`) and the fixes cover NM/ID/MS too. The report cards API serves **FY2018-2025** but the script took `fiscal_years[-1:]`, so six years were never fetched; `AZ_RC_YEARS` now controls scope and all eight years are on disk (~11 GB, ~71,500 JSON per year over state + 15,111 districts + 56,373 schools; 36-70% of responses are `[]`, more in the older years). A third bug surfaced here: **`IncrementalManifest` opened its file with `"w"`**, so a run scoped to one fiscal year truncated the manifest to that slice — the AZ report-card manifest described one year while eight sat on disk. It now carries prior rows forward and `finalize()` dedupes on a caller key; the class is shared with FL and three NV scripts, which had the same exposure. `az/rebuild_reportcards_manifest.py` rebuilds the record from the tree instead of re-downloading 11 GB. **On CRDC:** Arizona publishes no all-student discipline data — `safeandhealthy` is grant guidance and AzSAFE is a collection system, not a publication. Its own discipline source is SPP/APR **Indicator 4**, which is IDEA-scoped, LEA-level and reports Met/Not Met against a suspension-rate target (1,313 LEAs × 5 years) rather than counts by subgroup. So `discipline_crdc` is the only source of all-student suspension/expulsion counts for AZ and is load-bearing; the other `*_crdc` folders overlap more with state sources now that the state-side gaps are closed. Checked and skipped: `azed.gov/cte/data` (134 files, but upload templates and policy PDFs, not data). |
...
### Round-3 states (added 2026-08)

| State | Primary source(s) | Method | Entity depth | Notes |
|---|---|---|---|---|
| **Kansas** | `datacentral.ksde.gov/report_gen.aspx` | **Form replication** (ASP.NET → .xls) | state→county→district(→school) | The report generator has no JSON API but *is* a download endpoint: the final postback answers `Content-Type: application/vnd.ms-excel` with `%%_Reports.xls`, so we replicate the three-step postback chain (report → data group → year + Display Report) with plain requests and keep the body. 19 reports × {State, Totals by District/Organization, Totals by County} × 2015-2025 = 627 files. Most reports arrive pre-disaggregated: attendance rate by type **and race/ethnicity**, dropout and 4/5-yr cohort graduation **by race/ethnicity**, graduates by post-grad plans/**race**/**gender**, headcount enrollment by grade **and race/ethnicity**, plus FRL headcount, IDEA & gifted, suspension/expulsion counts and incidents, licensed and non-licensed personnel. Single-entity levels (one school / one district) exist but would need per-entity iteration and are not used. **TLS caveat:** KSDE serves an incomplete certificate chain — browsers repair it, Python does not — so verification is disabled for that host only (public data, no credentials). |
| **Kansas (assessment)** | `ksreportcard.ksde.gov` | **STATIC bulk files** + JSON service | state→district→**building** | Data Central publishes no test scores at all — they live in a second KSDE system. The Report Card's "Download Full Results (Large File · 28-68 MB)" link resolves to `https://ksreportcard.ksde.gov/<YYYY>_<YYYY+1>_Assessment_Full_File.xlsx`; **11 annual workbooks are published (2014-15 … 2024-25, 193 MB)** and they are the complete dataset — every level down to the individual building, every grade, subject and student subgroup. Two traps in the series. **(1)** Each workbook is a **two-year window**: `2023_2024` holds sheets `2023` *and* `2024`, `2024_2025` holds `2024` and `2025`. Every year but the endpoints is published twice and the copies are *not* identical, because KSDE revises — concatenating the eleven files naively double-counts eight of the ten years (3.17M rows where there are 2.21M). **(2)** KSDE reshaped the layout roughly every other year: **six schemas across eleven files**, headers renamed, columns reordered, the all-grades aggregate spelled `All Grades`, `ALL` or the bare code `13`. `scripts/ks/normalize_assessment.py` handles both — maps every layout onto one schema, then dedupes on (year, org, building, group, grade, subject, population) with the later workbook winning, while keeping rows an earlier edition has that the later one dropped. Result: `assessment_all_years.csv`, **2,209,296 rows**, 10 school years (2020 missing — COVID), 32 student groups, 2,329 org+building pairs. Entity codes: `0` = State of Kansas, `D####` = districts, `Z####` = the two Catholic diocesan systems. **Watch out:** 2015-16 and 2016-17 carry a `Population` column with both "Accountability" and "Report Card" rows — filter it or those years still double-count. **Gender** is published for 2014-15 and 2017-18 only; the other eight years carry race, poverty, disability, EL and mobility but no gender. The `dataService.svc/getPerfChart2016` JSON service is kept as a supplement only: the workbooks give **percentages**, the service gives **counts**. **Lesson:** the service endpoint was found first and taken as the answer, while the bulk-download link sat unread in the same probe output — the same miss as Minnesota's file-listers. |
| **New Mexico** | `web.ped.nm.gov` accountability + directory pages | **STATIC** | state→district→school | Best-disaggregated of this round: **232/233 files, 429 MB**, and **195 carry verified demographic columns** (ell 190, race 160, frl 151, **gender 148**, iep 124). Achievement-by-year publishes ELA/Math/Science proficiency as "attenuated" summaries plus By-Assessment and By-SubTest-&-Grade breakouts (privacy "Masked"); graduation publishes 4/5/6-year cohort rates back to cohort 2011. Plain WordPress uploads, no gating. |
| **Idaho** | `sde.idaho.gov/finance-transparency`; `idahoreportcard.org/datafiles` | **STATIC** (+ JS app) | state→district→building | **288/291 files, 554 MB** — ADA (full-term/midterm/best-28), support units, enrollment by building/district/grade, statewide enrollment summaries, revenues & expenditures 2004-2024, annual statements of financial condition, financial summaries, and staff salary reports. Note **162 of the files are PDFs** (notably the salary reports), which is why only 80 show verified demographic columns — a format limitation, not an absence. **The Idaho Report Card is also collected** (`scripts/id/reportcard.py`): its Blazor "Data Files" UI is awkward to drive, but the export behind it is an unauthenticated JSON endpoint — `POST idahoreportcard.org/api/DataExport/csv` with `{measures, breakdowns, yearIds, includeAllLevels}`. Numeric ids are unpublished; they were recovered by probing the endpoint and reading the returned `Measure Label` / `Student Group` columns, which revealed **38 measures (vs 18 in the UI** — the API also carries participation, proficiency-level bands, targets and IDAA variants) and **29 breakdowns including Male/Female**, which the UI hides in a nested picker. `includeAllLevels` returns state+district+school together. Oversized requests are refused ("Your selection is too large to export") so the breakdown list is halved recursively per measure-year. Result: **192/228 files, 5.7M rows, 823 MB** — the 36 gaps are legitimate (30 are 2019-20/COVID, 6 are advanced-math measures that post-date those years). |
| **Mississippi** | `mdek12.org/publicreporting` | **STATIC** | state→district→school | **178/178 files, 312 MB** across per-year pages (2018-19…2025-26) and the Assessment / Accountability / Reports / Diplomas / Staff topic pages. **Gotcha:** files served from `mdek12.org/sites/default/files/...` return **403 without a same-site `Referer`** — 20 files failed until the engine retried with one. 118 files are PDFs, so verified demographics sit at 35. `msrc.mdek12.org` (report cards) and `newreports.mdek12.org` are separate apps, not file listings, and are not crawled. |
